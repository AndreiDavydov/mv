...
Agreed after the first real run-through. Nothing here is done yet.

---

## 1. Rename "Tree" → "Catalog"
...
## 2. Packing something already in the catalog
...
Open question to settle when building: "recent" by `updated_at` across the whole
shared catalog, or only what *this device* has touched? Shared is more useful
with several helpers; per-device is less surprising. Leaning shared, with the
items this device just handled pinned to the top.

## 3. Drop rooms

Remove the room chips from enrolment and from edit, and stop offering `ROOMS` in
...
## 4. Drop `shelf` from container kinds
