> **This is the original brief, kept for reference.** Where the build departs
> from it — and it does, in several places — [DECISIONS.md](DECISIONS.md) says
> where and why. The largest departure: this document specifies a device-local
> catalog with "no accounts, no server, no sync" (§1, §11), and the catalog is
> now shared, hosted and private.
>
